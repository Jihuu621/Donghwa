Create Codex pet row `running-right` for `dubu`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve Dubu exactly from the existing spritesheet: a stylized human underground MC; retain face, attire, palette, silhouette, and any props. Upgrade only by adding natural, readable look directions.. Preserve silhouette, face, palette, material, proportions, markings, and props.
