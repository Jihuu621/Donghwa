Create one horizontal animation strip for Codex pet `dubu`, state `running-right`.
...
Identity: same pet in every frame: Preserve Dubu exactly from the existing spritesheet: a stylized human underground MC; retain face, attire, palette, silhouette, and any props. Upgrade only by adding natural, readable look directions.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
